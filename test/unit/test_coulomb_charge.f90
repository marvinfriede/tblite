      ...
      if (shell) then
         ...
         do isp = 1, mol%nid
            ...
            do ish = 1, shell_count(izp)
               ...
               write (*, *) "Hubbard ", izp, " ", ish, " ", hubbard_parameter(izp), shell_scale(ish, izp), hubbard(ish, isp)
            end do
         end do
         write (*, *) 'Hubbard parameters: ', hubbard
...
!> Factory to create electrostatic objects based on GFN1-xTB values
   ...
   subroutine test_e_effective_m02(error)
      ...
      call test_generic(error, mol, qat, qsh, make_coulomb_e1, 0.10635843572138280_wp)
